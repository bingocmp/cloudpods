...
func (self *SCloudaccount) importSubAccount(ctx context.Context, userCred mcclient.TokenCredential, subAccount cloudprovider.SSubAccount) (*SCloudprovider, bool, error) {
	...
	if subAccount.DefaultProjectId != "" {
		sProj, _ := identity.Projects.GetById(auth.GetAdminSession(ctx, ""), subAccount.DefaultProjectId, nil)
		if sProj != nil {
			domainId, _ = sProj.GetString("domain_id")
			projectId, _ = sProj.GetString("id")
		}
	}
